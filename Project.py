...
class Vehicle(pygame.sprite.Sprite):
    ...
    def move(self):#defines when the vehicle can move
        if(self.direction=='right'):
            if(self.crossed==0 and self.x+self.image.get_rect().width>stopLines[self.direction]):   # if crossed stop line now
                self.crossed = 1
            ...
            # (if the image has not reached its stop coordinate or has crossed stop line or has green signal) and (it is either the first vehicle in that lane or it
...
class Main:
    ...
    background = pygame.image.load('4wayintersection.png')

    screen = pygame.display.set_mode(screenSize)
    pygame.display.set_caption("SIMULATION")

    # Loading signal images and font
    redSignal = pygame.image.load('redlight.png')
    yellowSignal = pygame.image.load('yellowlight.png')
    greenSignal = pygame.image.load('greenlight.png')
